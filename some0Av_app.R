...
# Load model
load("bi_rr.fit_PSSTTWEA.rda")
load("bi_rr.bestlam_PSSTTWEA.R")
#load("TheMeans.R")
...
ui <- shinyUI(fluidPage(#
  ...
  sidebarLayout(#
    ...
    mainPanel(br(),br(),br(), br(),br(),br(),br(),br(),br(),br(),br(), br(),br(),"Based on reported historical observations and the inputs you supplied.", br(), strong("It is "), h1(textOutput("results1")), strong(" that you would encounter an avalance"), br(), br(), "For the curious, the raw output based on your inputs is", textOutput("results2"),br(), "Note: these are not probabilities.", width = 4)),
  "This model was created to provide another tool in evaluating danger in the backcountry.  The model was generated by datasets provided by the Utah Avalanche Center, and the NOAA.  The model was populated using ridge regression.  This particular model has a sensitivity of about 0.6 and a specificity of about 0.9, with a 95% confidence interval between 77% and 86% and a P-value of 5.099e-09.  If you would like to contribute please visit the project page at ", 
